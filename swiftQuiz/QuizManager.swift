...
class QuizManager {
    ...
    func refreshQuiz() {
        let randamIndex = Int(arc4random_uniform(UInt32(quizes.count)))
        let quizData = quizes[randamIndex]
        quiz = Quiz(question: quizData.question, options: quizData.options, correctedAnswers: quizData.correctAnswer)
    }
